...
You can also set Bookwise to open with any of these dialogs on startup via:
...
1. USB Mass Storage mode within Bookwise
...
Bookwise supports dictionary lookup in EPUB and even in scanned PDF/DJVU documents. To see the dictionary definition or translation, tap and hold a word.

To use the dictionary lookup function, first you need to install one or more dictionaries in the StarDict format. Bookwise has an inbuilt dictionary installation system:
...
language of the generated file and `v2015.11-985-g88308992` the Bookwise version
...
function QuickStart:getQuickStart()
    ...
    if lfs.attributes(quickstart_filename, "mode") ~= "file" then
        ...
        local quickstart_html = FileConverter:mdToHtml(quickstart_guide, _("Bookwise Quickstart Guide"), stylesheet)
        if quickstart_html then
            -- Fix links to images, which are in Bookwise install directory, which may not
